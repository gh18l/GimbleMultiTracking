...
bool ModelDetection::getFrameDetections(cv::Mat& frame, DETECTIONS& res, int aa) {
	...
	{
		box b = dets[i].bbox;
		int classname = max_index(dets[i].prob,l.classes);
		if(classname != 0) continue;
		float prob = dets[i].prob[classname];
		if(prob > thresh)
		{
			if(aa == 0)
			{
				int left	= (b.x - b.w/2.)*width_frame;
				int right	= (b.x + b.w/2.)*width_frame;
				int top 	= (b.y - b.h/2.)*height_frame;
				int bot 	= (b.y + b.h/2.)*height_frame;
				left 	= (left < 0? 0:left);
				right 	= (right>width_frame-1?width_frame-1:right);
				top		= (top < 0? 0:top);
				bot 	= (bot > height_frame-1?height_frame-1:bot);
				DETECTION_ROW tmpRow;
				tmpRow.aa = aa;
			...
			if(aa == 1)
			{
				int left	= (b.x - b.w/2.)*width_frame + frame.cols;
				int right	= (b.x + b.w/2.)*width_frame + frame.cols;
				int top 	= (b.y - b.h/2.)*height_frame;
				int bot 	= (b.y + b.h/2.)*height_frame;
				left 	= (left < 0? 0:left);
				right 	= (right>2*width_frame-1?2*width_frame-1:right);    //
				top		= (top < 0? 0:top);
				bot 	= (bot > height_frame-1?height_frame-1:bot);
				DETECTION_ROW tmpRow;
				tmpRow.aa = aa;
...
void ModelDetection::init(std::string cfgfile, std::string weightfile,float thr)
{
	this->loadFromFile = false;
	char* datacfg = "RUNNINGDATA/darknet_data/cfg/coco.data";
	//char* cfgfile = "RUNNINGDATA/darknet_data/cfg/yolov3.cfg";
	//char* weightfile = "RUNNINGDATA/darknet_data/yolov3.weights";
	//char* filename = "";
	thresh = thr;
	...
	net = parse_network_cfg_custom(const_cast<char*>(cfgfile.c_str()), 1);
...
std::vector<cv::Rect> ModelDetection::detect(cv::Mat frame)
{
	int width_frame = frame.cols;
	int height_frame = frame.rows;
	cv::Mat frame_temp;

													...
													calib::Imagefeature& current_feature, calib::Matchesinfo& matchesinfo,int i)
{
	...
	des2.upload(feature.des);     //��ʾÿ��ͼ���������������ÿ��������128ά��һ��ͼ��n������
										  // init l1 descriptor matcher
	...
	// find 1->2 matches
	pair_matches.clear();
	matcher->knnMatch(des1, des2, pair_matches, 2);   //pair_matches������ͼƬ������ƥ���
	for (size_t i = 0; i < pair_matches.size(); ++i) {
		if (pair_matches[i].size() < 2)
			continue;
		const cv::DMatch& m0 = pair_matches[i][0];
		const cv::DMatch& m1 = pair_matches[i][1];    //�ֱ��ʾ���ŶȰ������е�һ������ƥ��
		if (m0.distance < (1.f - matchConf) * m1.distance) {
			matchesinfo.matches.push_back(m0);     //���α�������ƥ��
			matches.insert(std::make_pair(m0.queryIdx, m0.trainIdx));
		}
	}
...
int calib::FeatureMatch::global2pano(cv::Mat global, cv::Mat pano)       //�������еĵ�ǰ��͵�ǰ����ʹ��̨�ƶ���Ŀ������λ��
{
	...
	des2.upload(pano_feature.des);     //��ʾÿ��ͼ���������������ÿ��������128ά��һ��ͼ��n������
								  // init l1 descriptor matcher
	cv::Ptr<cv::cuda::DescriptorMatcher> matcher =
		cv::cuda::DescriptorMatcher::createBFMatcher(cv::NORM_L1);
	// init variables for matching
...
int calib::FeatureMatch::read_features(std::vector<calib::Imagefeature>& feature)
{
	...
	para.open("/home/lgh/Documents/gimbleMulti/featurepara.txt", ios::binary | ios::in);
	if (!para.is_open())
	{
...
double calib::FeatureMatch::match_people(cv::Mat img1, cv::Mat img2)
{
	...
	des2.upload(img2_feature.des);     //��ʾÿ��ͼ���������������ÿ��������128ά��һ��ͼ��n������
									   // init l1 descriptor matcher
	cv::Ptr<cv::cuda::DescriptorMatcher> matcher =
		cv::cuda::DescriptorMatcher::createBFMatcher(cv::NORM_L1);
	// init variables for matching
